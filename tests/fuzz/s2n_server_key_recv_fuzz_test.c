...
/* Target Functions: s2n_server_key_recv s2n_kex_server_key_recv_read_data
   s2n_get_and_validate_negotiated_signature_scheme s2n_pkey_verify
   s2n_kex_server_key_recv_parse_data */
...
int LLVMFuzzerTestOneInput(const uint8_t *buf, size_t len)
{
    ...
    if (len >= 2) {
        ...
        GUARD(s2n_stuffer_read_uint8(&client_conn->handshake.io, &randval));
        client_conn->secure.cipher_suite = test_suites[randval % num_suites];

        /* Skip incompatible TLS 1.3 cipher suites */
        if (client_conn->secure.cipher_suite->key_exchange_alg == NULL) {
            GUARD(s2n_connection_free(client_conn));
            return 0;
        }

        client_conn->handshake_params.our_chain_and_key = cert;

        s2n_server_key_recv(client_conn);
