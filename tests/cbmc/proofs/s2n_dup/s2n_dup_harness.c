...
void s2n_dup_harness() {
    ...
    uint32_t old_from_size = from->size;
    uint8_t * old_from_ptr = from->data;
    uint32_t old_to_size = to->size;
    uint8_t * old_to_ptr = to->data;
    ...
    if (s2n_dup(from, to) == S2N_SUCCESS) {
        ...
        assert(old_from_size != 0);

        //This fails because checks are skipped
        assert(old_from_ptr != NULL);

        assert(old_to_size == 0);
        assert(old_to_ptr == NULL);
